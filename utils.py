...
def daytime(time,daycycle):
    ...
    try:
        ...
        settime  = daycycle[yr][dt]["sunset"]
        settime = parse(settime)
        risetime = daycycle[yr][dt]["sunrise"]
        risetime = parse(risetime)
        dctz     = daycycle['timezone']

        sunset = datetime.datetime(time.year, time.month, time.day, settime.hour, settime.minute, settime.second, settime.microsecond)
        sunrise = datetime.datetime(time.year, time.month, time.day, risetime.hour, risetime.minute, risetime.second, risetime.microsecond)

        dctz = pytz.timezone(dctz)
        sunset = dctz.localize(sunset)
        sunrise = dctz.localize(sunrise)
...
def get_for_id(id,table):
    ...
    if id != None:
        for x in range(len(table)):
            if table[x][0] == id: return table[x]
    else: return None
